...
nz_heights_sgbp <- st_intersects(x = nz_height, y = canterbury)
nz_height_logical <- lengths(nz_heights_sgbp) > 0 
...
tm_shape(cycle_hire) +
  tm_symbols(col = "firebrick", alpha = 0.2) +
tm_shape(cycle_hire_osm) +
  tm_symbols(col = "dodgerblue", alpha = 0.2)
```
...
if(nrow(cycle_hire) == nrow(cycle_hire_join)) {
  ...
  warning("join does not match orig. data dimensions")
}
```
...
us_west_union <- st_union(us_west) # combines each region to one polygon

texas <- us_states[us_states$NAME == "Texas", ]
texas_union <- st_union(us_west, texas)
...
```{r}
seine <- seine
seine_buffer <- st_buffer(seine, dist = units::set_units(5, "km"))
...
# same as st_fiilter with st_is_within_distance
